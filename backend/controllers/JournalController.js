...
class JournalController {
    ...
    static async createJournal(req, res) {
        ...
        try {
            ...
            res.status(201).json({ message: 'Journal Added Successfully', id: result.insertedId });
        } catch (error) {
    ...
    static async getAllJournal(req, res) {
        ...
        try {
            const quote = await dbClient.db.collection('journals').find({ userId: user._id }).toArray();
            if(!quote) {
                res.status(200).json({});
            }
    ...
    static async getJournalById(req, res) {
        ...
        try {
            const quote = await dbClient.db.collection('journals').findOne({ _id: new ObjectId(journalId), userId: user._id });
            if(!quote) {
                res.status(200).json({});
            }
            res.status(200).json({ quote });
    ...
    static async updateJournalById(req, res) {
        ...
        try{
            const result = await dbClient.db.collection('journals').updateOne(
            ...
            res.status(200).json({ message: `Journal entry ${journalId} updated successfully` });
        } catch (error){
    ...
    static async deleteJournalById(req, res) {
        ...
        try {
            const result = await dbClient.db.collection('journals').deleteOne({ _id: new ObjectId(journalId), userId: user._id });
            if (result.deletedCount === 0) {
                return res.status(404).json({ error: 'Quote not found or not authorized to delete' });
            }

            res.status(200).json({ message: `Quote ${journalId} deleted successfully` });
        } catch (error) {
    ...
    static async getMoodData(req, res) {
        ...
        try {
            const moodData = await dbClient.db.collection('journals').aggregate(pipeline).toArray();
